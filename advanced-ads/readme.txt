...
- Fix: Item object (ad/group) set parent affecting shared factory instance on placement abstract
- Fix: license screen download failed

= 2.0.24 (July 6, 2026) =

- Fix: UI fixes for Advanced Ads widget buttons on WP Dashboard on WP 7+
- Fix: a compatibility issue with group ad weights
...
- Fix: Ad label not applied to shortcode-rendered ads
- Fix: fatal error by correcting Quick_Bulk_Edit class namespace
- Fix: shortcode `ad_args` which allow remote code execution vulnerability
...
- Improvement: Added admin notice to warn when other ad-related plugins are active and may conflict with Advanced Ads
- Fix: remove license key prompt for Slider add-on
- Fix: clearfix checkbox saving and description alignment in the admin meta box
- Fix: prevent removal of backslashes in ad code
- Fix: screen options from Advanced Ads showing on other plugin’s pages
- Fix: conflict with Fluent-SMTP plugin
